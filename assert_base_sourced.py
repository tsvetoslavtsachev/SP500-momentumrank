#!/usr/bin/env python3
"""CI guard (INIT-22 P9): fail RED if SP500-momentumrank prices silently fell back to yfinance.

Coverage-floor + allow-list (the P6-part-2 / macro-satellite cardinality pattern), adapted to this
repo's DYNAMIC S&P 500 universe with a known quarantine gap (HON). The catastrophe this catches is a
SILENT MASS fallback (e.g. the archive checkout failed -> the whole universe quietly reverted to
un-audited yfinance). A handful of non-allow-listed fetches (a newly-added constituent not yet
registered -- index churn) are WARNED, not failed. Run only when the read PATs are set (the workflow
gates it); absent the secrets the yfinance fallback is legitimate and this is skipped.

NOTE currency: SP500-momentumrank is pure USD (normalize_currency=False, a no-op); this guard only
checks PROVENANCE (base vs fetch), not units. Fundamentals (marketCap/avgVolume) are a separate live
yfinance phase NOT covered by price_source.json -- only the price leg is asserted here.
"""
...
# the archive catalog. Documented in P8b-HON-case-to-resolve.md + the P9 mandate.
